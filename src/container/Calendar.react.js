...
import Text from '../../../../components/RobotoText';
...
type Props = {
  ...
  barArrowText?: Text.propTypes.style,
...
export default class Calendar extends Component {
  ...
  constructor(props: Props) {
    super(props);
    const stage =
      String(props.startStage) < String(props.finalStage)
        ? props.finalStage
        : props.startStage;
    this.state = {
      stage: stage,
  ...
  _previousStage = (): void => {
    if (this.props.changeStageDisable) {
      return;
    }
    if (this.state.stage === DAY_SELECTOR) {
  ...
  _changeFocus = (focus: Moment): void => {
    ...
    if (
      this.props.finalStage != DAY_SELECTOR &&
  ...
  render() {
    ...
    const previousMonthValid =
      this.props.minDate.diff(
        Moment(previousMonth).endOf("month"),
        "seconds"
      ) <= 0;
    const nextMonth = Moment(this.state.focus).add(1, "month");
    const nextMonthValid =
      this.props.maxDate.diff(Moment(nextMonth).startOf("month"), "seconds") >=
      0;
    ...
    return (
      ...
      >
          ...
          >
            ...
            previousMonthValid ? (
              <TouchableHighlight
                ...
                underlayColor={
                  barStyle ? barStyle.backgroundColor : "transparent"
                }
                onPress={this._previousMonth}
              >
                <Text style={this.props.barArrowText}>{LEFT_CHEVRON}</Text>
              </TouchableHighlight>
            ) : (
              <View />
            )}
            ...
            >
              <Text style={this.props.barText} font={fonts.fontBold} >{this._stageText()}</Text>
            </TouchableOpacity>
            ...
            nextMonthValid ? (
              ...
              >
                <Text style={this.props.barArrowText}>{RIGHT_CHEVRON}</Text>
              </TouchableHighlight>
            ) : (
              <View />
            )}
        ...
        <View style={[styles.stageWrapper, this.props.stageView]}>
          {this.state.stage === DAY_SELECTOR ? (
            <DaySelector
              ...
              dayTodaySelectText={this.props.dayTodaySelectText}
